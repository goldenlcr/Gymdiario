  ...
  const exsHTML=exs.map((ex,ei)=>{
    ...
    const demoHTML=`
      <div class="demo-panel${demoOpen[ei]?" open":""}" id="demo-${ei}">
        <div class="demo-img-wrap" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;background:#0d0d0d;border-radius:0" id="demo-yt-wrap-${ei}">
          <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:#333;font-size:10px;font-family:monospace" id="demo-loading-${ei}">Toca para cargar video</div>
          <div class="demo-svg-wrap" id="demo-svg-${ei}" data-ex="${ex.name}" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center">${makeSVGDemo(ex.name)}</div>
        </div>
        <div class="demo-tips">
          ${musclesHTML}
    ...
    btn.addEventListener("click",()=>{
      ...
      if(panel){
        ...
        if(demoOpen[ei]){
          ...
          const exData=EXERCISES[ex.name];
          if(wrap&&exData&&exData.youtube&&!wrap.querySelector("iframe")){
            svgEl&&(svgEl.style.display="none");
            loadEl&&(loadEl.style.display="none");
            const iframe=document.createElement("iframe");
            iframe.src=`https://www.youtube.com/embed/${exData.youtube}?rel=0&modestbranding=1`;
