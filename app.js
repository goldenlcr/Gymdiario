...
function initApp() {
  ...
  renderTrain();
  updateHeader();
  if(currentUser||true) loadFromCloud();
}
...
onAuthStateChanged(auth, user => {
  currentUser = user;
  if(user) {
    // User signed in — init app and load their cloud data
    const appEl = document.getElementById("app");
    if(!appEl.querySelector("#header")) {
      // Rebuild full app shell if we were on login screen
      location.reload();
    } else {
      loadFromCloud();
      renderTrain();
    ...
    // Not signed in — show login or continue as guest
  }
});
